...
def sample_and_group(npoint: int, radius: float, nsample: int, xyz: torch.Tensor,
                     points: torch.Tensor | None, returnfps: bool = False) -> tuple:
    B, N, C = xyz.shape
    S = npoint
...
def sample_and_group_all(xyz: torch.Tensor, points: torch.Tensor | None) -> tuple:
    device = xyz.device
    B, N, C = xyz.shape
...
class PointNetSetAbstraction(nn.Module):
    def __init__(self, npoint: int, radius: float, nsample: int, in_channel: int,
                 mlp: list[int], group_all: bool = False) -> None:
        super().__init__()
        self.npoint = npoint
        self.radius = radius
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1: torch.Tensor, xyz2: torch.Tensor,
                points1: torch.Tensor | None, points2: torch.Tensor | None) -> torch.Tensor:
