...
def get_labels(token, pc_id, start=0, end=100):
    ...
    if response.status_code == 200:
        ...
        print(f"[OK] Got labels: {labeled}/{len(data['labels'])} labeled points")
        return data
    else:
...
def export_point_cloud(token, pc_id, format="semantickitti"):
    ...
    if response.status_code == 200:
        print(f"[OK] Export {format} successful: {len(response.content)} bytes")
        return response.content
    else:
        print(f"[FAIL] Export {format} failed: {response.status_code} {response.text}")
        return None
